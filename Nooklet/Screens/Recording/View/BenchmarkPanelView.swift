...
struct BenchmarkPanelView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: columns, alignment: .leading, spacing: 8) {
                ...
                metric(
                    "RTF",
                    String(format: "%.3f", snapshot.realTimeFactor),
                    warn: snapshot.realTimeFactor >= 1.0
                )
                metric(
                    "Duration",
                    String(format: "%.1f s", snapshot.recordingSeconds)
                )
                metric("Mem (now)", mb(snapshot.currentMemoryMB))
                metric(
                    "Mem (peak)",
                    mb(snapshot.peakMemoryMB),
                    warn: snapshot.peakMemoryMB >= 1500
                )
                metric(
                    "Thermal",
                    snapshot.thermalState,
                    warn: snapshot.thermalState == "Serious"
    ...
    private func metric(_ label: String, _ value: String, warn: Bool = false)
        -> some View
    {
    ...
    private func mb(_ value: Double) -> String {
        value > 0 ? String(format: "%.0f MB", value) : "—"
    }
}
