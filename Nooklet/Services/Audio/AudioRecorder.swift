...
final class AudioRecorder {
    ...
    /// Normalised RMS level (0…1) emitted per buffer for UI visualisation.
    ...
    private func handle(buffer: AVAudioPCMBuffer) {
        ...
        // Compute RMS for UI audio level visualisation.
    ...
    /// Compute normalised RMS (0…1) from Float32 PCM samples.
    private static func computeRMS(_ samples: [Float]) -> Float {
        guard !samples.isEmpty else { return 0 }
        var sumOfSquares: Float = 0
        for s in samples { sumOfSquares += s * s }
        let rms = sqrtf(sumOfSquares / Float(samples.count))
        // Normalise: typical speech RMS ~0.01–0.1, clamp to 0…1.
